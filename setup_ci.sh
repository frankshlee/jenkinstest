...
sleep 20
java -jar /usr/share/jenkins/jenkins-cli.jar -s http://localhost:${JENKINS_PORT} install-plugin cvs subversion translation git github audit-trail createjobadvanced blame-upstream-commiters email-ext statusmonitor all-changes dry log-parser pmd violations ws-cleanup clamav ansicolor token-macro maven-plugin instant-messaging xcode-plugin skype-notifier growl ircbot greenballs simple-theme-plugin gerrit-trigger envinject
echo "If it just displayed \"Failed to authenticate with your SSH keys.\", please ignore."
sleep 60
java -jar /usr/share/jenkins/jenkins-cli.jar -s http://localhost:${JENKINS_PORT} restart
echo "If it just displayed \"Failed to authenticate with your SSH keys.\", please ignore."
